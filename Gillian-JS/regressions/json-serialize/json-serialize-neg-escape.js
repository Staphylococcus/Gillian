...
"use strict";
/* P09 negative control: json3 must escape control characters (\x01 -> \u0001).
   This asserts the WRONG (unescaped) form. The correct serializer escapes it,
   so the comparison is false and this MUST RFail. (guards against a naive
   serializer that omits escaping) */
var raw = String.fromCharCode(1);            // control char \x01
var ok = JSON.stringify(raw) === raw;        // correct output is the ESCAPED form, not raw
Assert(ok);                                   // must RFail
