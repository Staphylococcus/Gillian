#!/usr/bin/env python3
"""Object.getOwnPropertyNames interop controls (wpst / reduced InitFantine init).

Node oracle:
  Object.getOwnPropertyNames({a:1,b:2}).length == 2
Positive asserts the true length (must prove); negative asserts 3 (must reject
with a Pure assertion failure on the Assert line, not a cutoff or crash).
"""
...
CASES = [
    ("object-keys-positive", "wpst", None, True),
    ("object-keys-negative", "wpst", None, False),
]
...
if __name__ == "__main__":
    output = Path(tempfile.mkdtemp(prefix="gillian-object-interop-"))
    results = []
    for case in CASES:
